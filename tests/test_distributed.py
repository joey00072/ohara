...
def _worker(rank, rendezvous, mode, directory):
    ...
    try:
        ...
        if mode == "masked_ddp":
            ...
            inputs = torch.zeros(1, 3, dtype=torch.long)
            targets = torch.tensor([[0, -1, -1]] if rank == 0 else [[1, 1, 1]])
        ...
        if mode.startswith("tp"):
            ...
            engine.save(checkpoint, {"model": raw_model.state_dict(), "optimizer": optimizer.state_dict()})
            ...
            engine.load(checkpoint, {"model": model, "optimizer": optimizer})
...
@pytest.mark.parametrize("mode", ["tp", "tp_compiled", "masked_ddp", "moe_ddp"])
