...
toks = ds.map(tokenize_fn, batched = True, remove_columns=["text"], num_proc=128).shuffle(seed=42).filter(filter_fn, num_proc=128)
toks_val = ds_val.map(tokenize_fn, batched = True, remove_columns=["text"], num_proc=128).filter(filter_fn, num_proc=128)
