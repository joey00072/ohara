...
from modeling_pkv import ModelingLM, Config
...
print(f"{sys.argv=}")
for arg in sys.argv:
    if arg.startswith("attention_type="):
        attention_type = arg.split("=")[1]
    elif arg.startswith("head_dim="):
...
model_name = f"joey00072/exp-partial_kv-{attention_type}"
print(f"{model_name=}")
