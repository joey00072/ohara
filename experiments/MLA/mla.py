...
class MultiHeadLatentAttention(nn.Module):
    ...
    def __init__(self, config: Config):
        ...
        assert config.kv_lora_rank is not None, (
            f"kv_lora_rank is not defined {config.kv_lora_rank=}"
        )
        assert config.rope_head_dim is not None, (
            f"rope_head_dim is not defined {config.rope_head_dim=}"
        )
        ...
        self.scale = 1 / (self.nope_dim + self.rope_dim) ** 0.5
    ...
    def forward(self, x: Tensor, mask: torch.Tensor, freqs_cis: Tensor):
        ...
        query_nope = query_nope.view(
            batch_size, seq_len, self.num_heads, self.nope_head_dim
        ).transpose(1, 2)
        query_rope = query_rope.view(
            batch_size, seq_len, self.num_heads, self.rope_head_dim
        ).transpose(1, 2)

        key_rope = key_rope.view(batch_size, seq_len, 1, self.rope_head_dim).transpose(1, 2)
        key_nope = key_nope.view(batch_size, seq_len, self.num_heads, self.nope_head_dim).transpose(
            1, 2
        )
    ...
    def reset_parameters(
        self, init_std: float | None = None, factor: float = 1.0, qkv_std: float = None
    ) -> None:
        init_std = init_std or (self.dim ** (-0.5))
        qkv_std = qkv_std or (self.v_head_dim ** (-0.5))

        for w in [
            self.compress_q_linear,
            self.decompress_q_nope,
            self.decompress_q_rope,
            self.k_rope_linear,
        ]:
...
class MLAInference(MultiHeadLatentAttention):
    def __init__(self, config: Config):
    ...
    def forward(self, x: Tensor, freqs_cis: Tensor):
        ...
        def _test_self_attention(x: Tensor):
            ...
            q_recombined = torch.empty(
                (batch_size, self.num_heads, seq_len, self.head_dim), device=x.device
            )
            k_recombined = torch.empty(
                (batch_size, self.num_heads, seq_len, self.head_dim), device=x.device
            )
            ...
            output = F.scaled_dot_product_attention(
                q_recombined, k_recombined, value, is_causal=True
            )
...
class DSMultiHeadLatentAttention(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.kv_a_proj_with_rope = nn.Linear(
            self.dim, self.kv_lora_rank + self.rope_head_dim, bias=False
        )
    ...
    def forward(self, x: torch.Tensor, mask: torch.Tensor, freqs_cis: torch.Tensor):
        ...
        compressed_kv, k_rope = torch.split(
            kv_proj, [self.kv_lora_rank, self.rope_head_dim], dim=-1
        )

        norm_kv = self.kv_a_layernorm(compressed_kv)
        kv = self.kv_b_proj(norm_kv)
        kv = kv.view(
            batch_size, seq_len, self.num_heads, self.nope_head_dim + self.head_dim
        ).transpose(1, 2)
        ...
        attn_output = F.scaled_dot_product_attention(
            q_recombined, k_recombined, v, attn_mask=mask, dropout_p=self.dropout, is_causal=True
        )
...
def mla_reformulation_test(config: Config):
    ...
    mla_inference = MLAInference(config)
...
if __name__ == "__main__":
    ...
    print(
        f"Model Size: {sum(p.numel() for p in mla.parameters()) / 1e6}M params, attn size {d_model * d_model * 4 / 1e6}m"
    )
